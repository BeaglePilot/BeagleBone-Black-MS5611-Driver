...
void      MS5611_read_pt   (struct spi_device *, MS5611Report *);
...
int MS5611_remove(struct spi_device *spi) {
  if(!spi) {
    printk(KERN_ALERT "MS5611_remove(): pointer corruption\n");
    return -1;
  }
...
void MS5611_read_pt(struct spi_device *spi, MS5611Report *pRep) {
  ...
  pRep->temperature_Deg = 2000 + dT * _pCOEF[6] / (1 << 23);
  
  //SECOND ORDER TEMP COMPENSATION
  if(pRep->temperature_Deg < 2000) {
    T2    = (dT *dT) >> 31;
    TEMP  = pRep->temperature_Deg - 2000;
    OFF2  = (5 *  TEMP * TEMP) >> 1;
    SENS2 = OFF2 >> 1;
    
    if(pRep->temperature_Deg < -1500) {
      TEMP  = pRep->temperature_Deg + 1500;
      OFF2  = OFF2 + 7 * TEMP * TEMP;
      SENS2 = (SENS2 + (11 * TEMP * TEMP) ) >> 1;
    }
    
    pRep->temperature_Deg -= T2;
    OFF   -= OFF2;
    SENS  -= SENS2;
  }

  pRep->pressure_Pa = (int32_t)((D1 * SENS / (1 << 21) - OFF) / (1 << 15) );
}
